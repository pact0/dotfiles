...
ls.snippets = {
    ...
    lua = {
        snip("shebang", {text {"#!/usr/bin/lua", "", ""}, insert(0)}),
        snip("req",
             {text "require('", insert(1, "Module-name"), text "')", insert(0)}),
        snip("func", {
            text "function(", insert(1, "Arguments"), text {")", "\t"},
            insert(2), text {"", "end", ""}, insert(0)
        }), snip("forp", {
            text "for ", insert(1, "k"), text ", ", insert(2, "v"),
            text " in pairs(", insert(3, "table"), text {") do", "\t"},
            insert(4), text {"", "end", ""}, insert(0)
        }), snip("fori", {
            text "for ", insert(1, "k"), text ", ", insert(2, "v"),
            text " in ipairs(", insert(3, "table"), text {") do", "\t"},
            insert(4), text {"", "end", ""}, insert(0)
        }), snip("if", {
    ...
    markdown = {
        ...
        snip({
            ...
            text "``` ", insert(1, "Language"), text {"", ""},
            insert(2, "Content"), text {"", "```", ""}, insert(0)
        }), snip({
            trig = "meta",
            namr = "Metadata",
            ...
            func(date, {}), text {"", "cathegories: ["}, insert(3, ""),
    ...
    go = {
        snip("test", {
            text "func ", insert(1, "Name"), text "(t *testing.T)",
            text {" {", ""}, text "\t", insert(0), text {"", "}"}
        }), snip("tyi", {
            text "type ", insert(1, "Name"), text {" interface {", ""},
            text "\t", insert(0), text {"", "}"}
        }), snip("tys", {
            ...
            insert(3), text {" {", ""}, text "\t", insert(0), text {"", "}"}
        }), snip("if", {
            text "if ", insert(1, "true"), text {" {", ""}, text "\t",
            insert(0), text {"", "}"}
        }), snip("fori", {
            text "for ", insert(1, "i := 0"), text ";", insert(2, "i < 10"),
            text ";", insert(3, "i++"), text {" {", ""}, text "\t", insert(0),
            text {"", "}"}
        }), snip("forr", {
            text "for ", insert(1, "k, v"), text " := range ",
            insert(2, "expr"), text {" {", ""}, text "\t", insert(0),
            text {"", "}"}
        })
    }
}
